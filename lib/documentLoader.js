...
async function _didEbsiLoader(url) {
  console.log('Entering ebsi loader');
  if(!url.startsWith('did:ebsi:')) {
    throw new Error('NotFoundError');
  }

  const [did, fragment] = url.split('#');
  console.log('Found the following', {did, fragment});
  let document;
  try {
    ...
    if(fragment) {
      ...
      console.log('plucked did node result:', document);
...
async function _pluckDidNode(target, didDocument, context) {
  console.log('pluck opts', {target, didDocument, context});
  ...
  console.log('framed opts', {filtered, context, target});
  console.log('framed result', framed);
