...
function addRoutes(app) {
  app.post(cfg.routes.verify, brRest.when.prefers.ld,
    asyncHandler(async (req, res) => {
      ...
      let suite;
      if(presentation.proof.type === 'Ed25519Signature2020') {
        suite = new Ed25519Signature2020();
      } else if(presentation.proof.type === 'Ed25519Signature2018') {
  ...
  app.post('/verifier/credentials', cors(), asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if(credential.proof.type === 'Ed25519Signature2020') {
        suite = new Ed25519Signature2020();
      } else if(credential.proof.type === 'Ed25519Signature2018') {
        suite = new Ed25519Signature2018();
      }
  ...
  app.post('/verifier/presentations', cors(), asyncHandler(async (req, res) => {
    ...
    try {
      ...
      let suite;
      if(verifiablePresentation.proof.type === 'Ed25519Signature2020') {
        suite = new Ed25519Signature2020();
      } else if(verifiablePresentation.proof.type === 'Ed25519Signature2018') {
        suite = new Ed25519Signature2018();
      }
      _validateChecks({checks});
